...
def asteroid_closest_approach():
    ...
    #to take care of the API limit, setting a page limit to 5
...
def sorting(asteroids, closest_asteroids):
    for asteroid in asteroids['near_earth_objects']:
        ...
        if asteroid['close_approach_data']:
            asteroid['close_approach_data'] = min(asteroid['close_approach_data'], \
                key=lambda x:x['miss_distance']['astronomical'])
